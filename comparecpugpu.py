...
def measure_inference_time(model, device, input_text, src_code, tgt_code):
    """
    Measure the time taken for inference on a specific device.
    """
    ...
    try:
        ...
        return " ".join(translations).strip(), duration
    except Exception as e:
...
def compare_inference_time(input_text, src_code, tgt_code):
    """
    Compare inference times for GPU and CPU.
    """
    logger.info("Comparing inference times between GPU and CPU...")

    # GPU inference
    logger.info("Running inference on GPU...")
    gpu_translation, gpu_time = measure_inference_time(model_gpu, device_gpu, input_text, src_code, tgt_code)

    # CPU inference
    logger.info("Running inference on CPU...")
    cpu_translation, cpu_time = measure_inference_time(model_cpu, device_cpu, input_text, src_code, tgt_code)
